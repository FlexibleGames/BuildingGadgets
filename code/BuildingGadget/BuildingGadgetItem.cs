            ...
            if (toolMode != 2)
            {
                for (int x = minBlock.X; x <= maxBlock.X; x++)
                {
                    for (int y = minBlock.Y; y <= maxBlock.Y; y++)
                    {
                        ...
                        {
                            blocksForTool.Add(new BlockPos(x, y, z));
                        }
                    }
                }
            }
            else
            {
                ...
                for (int x = minBlock.X; x <= maxBlock.X; x++)
                {
                    for (int y = minBlock.Y; y <= maxBlock.Y; y++)
                    {
                        for (int z = minBlock.Z; z <= maxBlock.Z; z++)
                        {
                            Block blockCheck = api.World.BlockAccessor.GetBlock(x, y, z);
                            if (blockCheck.Id == 0) continue; // skip air
                            // ignore blocks with an entity
                            if (api.World.BlockAccessor.GetBlockEntity(new BlockPos(x, y, z)) == null) 
                            {
